...
export class Building {
  ...
  public update(): void {
    ...
    // Update waiting people on floors and handle button represses and people giving up
    for (let floor = 0; floor < this.floors; floor++) {
      const peopleToRemove:number[] = []; // Track people who've given up
      let buttonRepressNeeded = false;
      
      this.waitingPeople[floor].forEach((person, index) => {
        person.updateWaitingTime();
        
        // Check if anyone needs to press the button again
        if (person.isButtonRepressNeeded) {
          buttonRepressNeeded = true;
          person.resetButtonPressFlag();
        }
      ...
      // Handle button represses
      if (buttonRepressNeeded) {
        console.log(`Button on floor ${floor} was pressed again by an impatient person`);
        this.reassignPeopleOnFloor(floor);
        this.floorButtonPressTime[floor] = this.p.millis(); // Reset the button press time
      }
      ...
      for (let i = peopleToRemove.length - 1; i >= 0; i--) {
        ...
        // Add to a special "took stairs" counter if you want to track this metric
  ...
  private loadPeopleToElevator(elevatorIndex: number, floor: number): void {
    ...
    for (const person of peopleForThisElevator) {
      if (elevator.addPerson(person)) {
        ...
        console.warn(`Elevator ${elevatorIndex + 1} could not accept person going to floor ${person.DestinationFloor}`);
      }
    }
  ...
  private reassignPeopleFromBrokenElevator(brokenElevatorIndex: number, floor: number): void {
    ...
    if (peopleToReassign.length > 0) {
      console.log(`Reassigning ${peopleToReassign.length} people from broken elevator ${brokenElevatorIndex + 1}`);
  ...
  /**
   * Reassign all people on a floor to potentially better elevators
   * Called when someone presses the button again
   */
  private reassignPeopleOnFloor(floor: number): void {
    const floorStats = this.getFloorStats();
    const peopleOnFloor = this.waitingPeople[floor];
    
    // Only reassign people who don't have an elevator in LOADING state at their floor
    const peopleToReassign = peopleOnFloor.filter(person => {
      const elevatorIndex = person.getAssignedElevator();
      if (elevatorIndex === -1) return true; // No elevator assigned
      
      const elevator = this.elevators[elevatorIndex];
      return !(elevator.isAtFloor() && elevator.getCurrentFloor() === floor);
    });
    
    if (peopleToReassign.length > 0) {
      console.log(`Reassigning ${peopleToReassign.length} people on floor ${floor} due to button repress`);
      
      peopleToReassign.forEach(person => {
        // Find a new elevator (might be the same one)
        const newElevatorIndex = this.elevatorSystem.assignElevatorToPerson(person, floor, floorStats);
        
        // Update the person's assigned elevator
        person.setAssignedElevator(newElevatorIndex);
        
        // Make sure the elevator knows to visit this floor
        this.elevators[newElevatorIndex].addFloorToVisit(floor);
      });
    }
  }
  ...
  public getStatistics() {
    ...
    let totalWaitTime = 0;
    let count = this.servedPeople.length;
    
    this.servedPeople.forEach(person => {
      totalWaitTime += person.WaitTime;
    });
    
    const avgWaitTime = count > 0 ? totalWaitTime / count : 0;
    
    // Calculate efficiency score (inverse of wait time, normalized)
    const efficiency = count > 0 ? 1000 / (avgWaitTime + 1) : 0;
